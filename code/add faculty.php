...
<input type="text" name="post" placeholder="ENTER FACULTY POST" style="position:absolute;top:360;left:500px;height:35px;width:215px" required>
<input type="text" name="password" placeholder="ASSIGN A PASSWORD TO FACULTY" style="position:absolute;top:360;left:800px;height:35px;width:215px" required>
...
$post=$_POST['post'];
$pwd=$_POST['password'];
if(empty($na) or empty($mobile) or empty($address) or empty($post) or empty($email) or empty($pwd))
{
...
$num=mysqli_num_rows($q1);
	if($num>=1)
	{
	echo'<h3 style="position:absolute;top:460px;left:500px;color:red;font-family:algerian"><u><blink>'."the faculty is in institute"."</blink></h3>";
	}
	else
	{

	$query="insert into faculty values('','$na','$address',$mobile,'$email','$post','$pwd');";
$q=mysqli_query($conn,$query);
		if($q)
		{
	echo'<h3 style="position:absolute;top:460px;left:550px;color:red;font-family:algerian"><u><blink>'."faculty added"."</blink></h3>";
